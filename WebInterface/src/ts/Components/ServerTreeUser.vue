    ...
    <div class="channel-line">
      ...
      >
        <div class="container" style="display: flex; align-items: center">
          <span>
            <b-icon
              :icon="node.Id == own_client.Id ? 'robot' : 'account'"
            ></b-icon>
          </span>
...
export default Vue.component("server-tree-user", {
  ...
  computed: {
    ...
    playing_here(): boolean {
      if (this.meta.send_mode.SendMode === TargetSendMode.Voice) {
        ...
        return this.meta.send_mode.WhisperClients.includes(this.node.Id);
      }
      return false;
    },
  },
  methods: {
    async voiceHere() {
      const res = await bot(cmd<void>("whisper", "off"), this.botId).get();
    ...
    async botRename(name: string) {
      const res = await bot(cmd<void>("bot", "name", name), this.botId).get();
