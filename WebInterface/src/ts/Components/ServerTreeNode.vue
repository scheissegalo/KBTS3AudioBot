...
import { IChannelBuildNode } from "./ServerTreeTypes";
...
export default Vue.component("server-tree-node", {
  ...
  computed: {
    ...
    channel(): { type: SpacerType; name: string } {
      ...
      if (!this.root) return { type: SpacerType.None, name: channel.Name };
      const nameSpacer = /^\[(c|r|\*|)spacer[^\]]*\](.*)$/.exec(channel.Name);
      if (nameSpacer == null)
    ...
    playing_here(): boolean {
      if (this.meta.send_mode.SendMode === TargetSendMode.Voice) {
        return false;
      } else if (this.meta.send_mode.SendMode === TargetSendMode.Whisper) {
        return this.meta.send_mode.WhisperChannel.includes(this.node.own.Id);
      }
      return false;
    },
  ...
  methods: {
    async changeChannel() {
      const channel = this.node.own;
      if (channel.Id === 0 || channel.Id === this.own_client.Channel) return;
    ...
    async whisperHere() {
      const channel = this.node.own;
      const subCmd = cmd<void>("subscribe", "channel", String(channel.Id));
      const isWhisper = this.meta.send_mode.SendMode !== TargetSendMode.Whisper;
      let res = await bot(
        isWhisper
          ? (jmerge(cmd<void>("whisper", "subscription"), subCmd) as Api<void>)
          : subCmd,
        this.botId
      ).get();
